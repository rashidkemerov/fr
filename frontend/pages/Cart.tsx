       ...
       <div className="p-4 space-y-4">
         {cart.length === 0 ? (
           <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
             ...
             <button 
               onClick={() => navigate('/')} 
               className="bg-accent text-white font-bold py-3 px-8 rounded-2xl shadow-lg shadow-orange-200 active:scale-95 transition-transform"
             >
               Перейти в меню
             </button>
           </div>
           ...
           cart.map((item, idx) => {
             ...
             return (
               <div key={`${item.productId}-${item.selectedOptionId}`} className="bg-white rounded-[1.5rem] p-3 shadow-sm border border-slate-100 flex gap-4 items-center animate-fade-in">
