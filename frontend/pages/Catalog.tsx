...
const Catalog: React.FC = () => {
  ...
  // Combined Scroll Handler (ScrollSpy + Smart Header + Close Dropdown)
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (currentScrollY > 60) {
        ...
        setShowTopBar(true); // At Top -> Show Top Bar
      }
      lastScrollY.current = currentScrollY;

      // 2. Close Dropdown on scroll
      if (openSubmenu) setOpenSubmenu(null);

      // 3. ScrollSpy Logic
      if (!isScrollingRef.current) {
        // Offset accounts for the visible header height (dynamic)
        const offset = showTopBar ? 180 : 120;
  ...
  const scrollToSection = (refMap: React.MutableRefObject<Record<string, HTMLDivElement | null>>, key: string) => {
    ...
    if (element) {
      ...
      // If we scroll up to a category, the header might expand, so we stick to a safe offset
  ...
  const handleCategoryClick = (catName: string, e: React.MouseEvent) => {
    ...
    if (hasSubs) {
      ...
      setDropdownPos({ top: rect.bottom + 10, left: rect.left }); // Add slight buffer
  ...
  return (
    <div className={`min-h-screen pb-28 pt-[180px] transition-colors duration-300 ${theme === 'dark' ? 'bg-slate-900' : 'bg-[#F8FAFC]'}`} onClick={() => setOpenSubmenu(null)}>
      
      {/* Fixed Header Wrapper */}
      <div 
        className={`fixed top-0 left-0 right-0 z-30 shadow-sm transition-all duration-300 border-b backdrop-blur-xl ${theme === 'dark' ? 'bg-slate-900/95 border-slate-800' : 'bg-[#F8FAFC]/95 border-slate-200'}`}
        style={{ transform: showTopBar ? 'translateY(0)' : 'translateY(-64px)' }} 
      >
         
         {/* Top Section (Logo & Admin) - Height approx 64px */}
         <div className="h-[64px] flex justify-between items-center px-4 py-3">
           ...
           <div className="flex gap-2">
             <button onClick={(e) => { e.stopPropagation(); setIsSettingsOpen(true); }} className={`w-10 h-10 rounded-full flex items-center justify-center active:scale-95 transition-transform ${theme === 'dark' ? 'bg-slate-800 text-white' : 'bg-white text-primary shadow-sm'}`}>
                <Palette size={20} strokeWidth={2} />
             </button>
             <button onClick={() => navigate('/admin')} className={`w-10 h-10 rounded-full flex items-center justify-center active:scale-95 transition-transform ${theme === 'dark' ? 'bg-slate-800 text-white' : 'bg-white text-primary shadow-sm'}`}>
                <Settings size={20} strokeWidth={2} />
             </button>
             <button onClick={() => navigate('/history')} className={`w-10 h-10 rounded-full flex items-center justify-center active:scale-95 transition-transform ${theme === 'dark' ? 'bg-slate-800 text-white' : 'bg-white text-primary shadow-sm'}`}>
               <History size={20} strokeWidth={2} />
             </button>
           </div>
         </div>

         {/* Search Section - Permanently Visible */}
         <div className="px-4 pb-2 pt-1">
            <div className="relative group">
              <input 
                type="text" placeholder="Поиск блюд..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                className={`w-full rounded-2xl py-2.5 pl-10 pr-4 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-accent/50 transition-all ${theme === 'dark' ? 'bg-slate-800 text-white border-slate-700' : 'bg-white text-slate-900 border-gray-100 shadow-sm'}`}
              />
         ...
         {/* Categories Navigation - Permanently Visible */}
            ...
            return (
              <div key={cat.id} className="relative flex-shrink-0">
                <button
                  ...
                  className={`flex items-center gap-1 whitespace-nowrap px-4 py-2 rounded-2xl text-xs font-bold uppercase tracking-wider transition-all duration-300 border ${
                    isActive
        ...
        >
          {categoryStructure[openSubmenu].map(sub => (
            <button
              ...
              className={`text-left px-3 py-2.5 rounded-xl text-xs font-bold transition-all ${theme === 'dark' ? 'text-slate-300 hover:bg-slate-700/50 hover:text-white' : 'text-slate-600 hover:bg-slate-100 hover:text-primary'}`}
            >
              {sub}
            </button>
          ))}
      ...
      <div className={`fixed bottom-0 left-0 w-full p-4 z-40 transition-transform duration-300 ${totalItems > 0 ? 'translate-y-0' : 'translate-y-[120%]'}`}>
        <div className="glass-panel rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] p-2 border border-white/40 backdrop-blur-2xl">
          <div className="flex justify-between items-center bg-primary rounded-2xl p-1 pr-2 shadow-lg overflow-hidden relative">
            ...
            <button onClick={() => navigate('/cart')} className="bg-white text-primary rounded-xl px-6 py-3 font-bold uppercase tracking-wider flex items-center gap-3 active:scale-95 transition-transform relative z-10">
              Заказать 
              <span className="bg-accent text-white text-xs px-2 py-0.5 rounded-md font-black min-w-[20px] text-center">{totalItems}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={() => setIsSettingsOpen(false)}>
           <div className={`w-full max-w-sm rounded-3xl p-6 animate-slide-up shadow-2xl ${theme === 'dark' ? 'bg-slate-800 text-white border border-slate-700' : 'bg-white text-slate-900'}`} onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-6">
                <div>
                   ...
                   <div className="grid grid-cols-2 gap-2">
                      <button onClick={() => setTheme('light')} className={`p-3 rounded-xl border-2 flex items-center justify-center gap-2 font-bold transition-all ${theme === 'light' ? 'border-accent bg-orange-50/50 text-accent' : 'border-transparent bg-gray-100/5'}`}><Sun size={18} /> Светлая</button>
                      <button onClick={() => setTheme('dark')} className={`p-3 rounded-xl border-2 flex items-center justify-center gap-2 font-bold transition-all ${theme === 'dark' ? 'border-accent bg-slate-700 text-accent' : 'border-transparent bg-gray-100/5'}`}><Moon size={18} /> Темная</button>
                   </div>
                </div>
                ...
                <div>
                   ...
                   <div className="grid grid-cols-3 gap-2">
                      {(['small', 'medium', 'large'] as FontSize[]).map((size) => (
                        <button key={size} onClick={() => setFontSize(size)} className={`p-3 rounded-xl border-2 flex flex-col items-center justify-center gap-1 font-bold transition-all ${fontSize === size ? 'border-accent bg-orange-50/10 text-accent' : 'border-transparent bg-gray-100/5'}`}>
...
const CatalogSkeleton = () => (
  <div className="min-h-screen bg-[#F8FAFC] px-4 py-4 space-y-6">
    ...
    <div className="grid grid-cols-2 gap-4">
       {[1,2,3,4,5,6].map(i => <div key={i} className="h-48 bg-slate-200 rounded-2xl animate-pulse"></div>)}
    </div>
  </div>
);
